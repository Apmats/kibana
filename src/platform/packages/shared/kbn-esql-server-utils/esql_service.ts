...
export class EsqlService {
  ...
  public async getColumns(
    ...
    const { columns, all_columns: allColumns } = response as typeof response & {
      all_columns?: EsqlColumn[];
    };
    const responseColumns: EsqlColumn[] = allColumns ?? columns;
    return responseColumns.map(({ name, type, original_types: originalTypes }) => {
